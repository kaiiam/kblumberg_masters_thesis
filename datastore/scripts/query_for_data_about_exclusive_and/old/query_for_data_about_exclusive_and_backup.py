...
# taking only the subje[x] go and get the data assocaited with that colmn/row
# c_values = []
# for x in return_indices:
#     c_values.append(subj[x].pop())
...
#     print subj_2[x].pop(), val[x]
#
#
# #print only the rows with satisty the AND statment.
# for x in return_indices:
#     print subj[x].pop(), fobj[x], pred[x]
...
# using len, I can generate the stats how how well the full vs partial querying models perform.
# for x in return_indices:
#     print subj[x].pop(), len(fobj[x]), len(pred[x])
...
# f = open(outstring, 'w')
# for x in return_indices:
#     f.write( str( subj[x].pop() ) + ',' +  str(len(fobj[x])) + ',' + str(len(pred[x])) + '\n' )
